/* js wirting document order */
/* 1) GENERAL DEFINITIONS & SETUP (to do before running any other functions) */
...
// eventlistener has the same function of a css on click attribute
...
/* event listener exmaple (listen out for an event that's gonna happen  when specified in between brackets). WHenever mouse hovers, run function logEvent*/
...
function playPause(){
  ...
  if (videoElement.paused || videoElement.ended) //ended state is when video ends completely
   {
...
/* last: HELPER FUNCTIONS (generic function u can reuse throughout projects)*/
